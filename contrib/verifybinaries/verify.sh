...
###   dollarcore.org and dollar.org and compares them.
...
WORKINGDIR="/tmp/dollar_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://dollarcore.org"
HOST2="https://dollar.org"
BASEDIR="/bin/"
VERSIONPREFIX="dollar-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'dollar-',
...
if [ $? -ne 0 ]; then
   echo "dollar.org failed to provide signature file, but dollarcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "dollar.org and dollarcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Dollar Core binary release signing key installed?"
   fi
