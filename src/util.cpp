...
const char * const DOLLAR_CONF_FILENAME = "dollar.conf";
const char * const DOLLAR_PID_FILENAME = "dollard.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "dollard -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Dollar
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Dollar
    // Mac: ~/Library/Application Support/Dollar
    // Unix: ~/.dollar
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Dollar";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Dollar";
#else
    // Unix
    return pathRet / ".dollar";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", DOLLAR_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", DOLLAR_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoin Core") == std::string::npos) {
        ...
        strYear.replace(strYear.find("2021"), sizeof("2021")-1, "2009-2021");
